...
use App\Models\User;
use App\Notifications\BlankProductionReportNotification;
use Carbon\Carbon;
use Exception;
    ...
    public function handle() {
        ...
        foreach ( $quarters as $quarter => $startDate ) {
            ...
            if ( "01-01-2025" === $startDate ) {
                ...
                $growers = Grower::whereJsonContains( 'production_reporting_quarter', $quarter )->get();

                $this->info( "Found {$growers->count()} growers for quarter {$quarter}" );

                foreach ( $growers as $grower ) {
                    $existingReport = ProductionReport::where( 'grower_id', $grower->id )
                    ...
                    $quartersArray = json_decode( $grower->production_reporting_quarter, true );
                    ...
                    $link = env( 'FRONTEND_URL' ) . "/production-reports";
                    ...
                    try {
                        ...
                        $user->notify( new BlankProductionReportNotification( $grower, $quarter_uc, $aboutQuarters, $link, $currentYear ) );
