...
class SalesReportExport implements WithDrawings, WithEvents, WithStyles {
    protected $salesReports;
    protected $grower;

    public function __construct( $salesReports, $grower ) {
    ...
    public function registerEvents(): array {
        return [
            AfterSheet::class => function ( AfterSheet $event ) {
                ...
                $sheet->setCellValue( 'C24', 'Sold numbers' );
                $sheet->setCellValue( 'H24', 'Year:' );
                $sheet->setCellValue( 'I24', ( $this->salesReports->year ) . ' - ' . ucwords( $this->salesReports->quarter ) );

                $sheet->setCellValue( 'C25', 'VARIETY' );
                $sheet->setCellValue( 'H25', 'Quantity' );

                $reportData = json_decode( $this->salesReports->data, true );
                $rowIndex = 26;
                foreach ( $reportData as $data ) {
                    $sheet->setCellValue( 'C' . $rowIndex, $data['product_name'] );
                    $sheet->setCellValue( 'H' . $rowIndex, $data['amount'] );
                    $sheet->setCellValue( 'I' . $rowIndex, "€ " . number_format( $data['unit_price'], 2 ) );
    ...
    private function formatQuartersArray(): string {
        $quartersArray = json_decode( $this->salesReports->quarters_array, true );
