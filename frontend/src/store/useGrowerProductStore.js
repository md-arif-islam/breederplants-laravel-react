...
export const useGrowerProductStore = create((set, get) => ({
    ...
    getAllGrowerProducts: async (page = 1, searchQuery = "", growerId = "") => {
        ...
        try {
            ...
            const message =
                ...
                "Error fetching grower products";
            toast.error(message);
        } finally {
            set({ isLoading: false });
        }
    ...
    addQuantityToStock: async (growerId, id, quantity) => {
        ...
        try {
            ...
            const message =
                error?.response?.data?.message || "Error adding quantity";
