...
export const useSalesReportStore = create((set, get) => ({
  ...
  getAllSalesReportsForAdmin: async (
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Error fetching sales reports";
      toast.error(message);
    } finally {
      set({ isLoading: false });
    }
  ...
  getSalesReportById: async (id) => {
    set({ isLoading: true });
    try {
      const res = await axiosInstance.get(`/api/admin/sales-reports/${id}`);
  ...
  submitSalesReport: async (data) => {
    ...
    try {
      ...
      console.log(error);
      const message =
        error?.response?.data?.message || "Error submitting sales report";
      toast.error(message);
    } finally {
      set({ isLoading: false });
    }
