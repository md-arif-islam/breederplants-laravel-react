              ...
              <tbody>
                ...
                {!isLoading &&
                  ...
                  salesReports.map((report) => {
                    ...
                    const dateOpen = dateRange ? dateRange.start : null;
                    return (
                      <tr
                        key={report.id}
                        className="hover:bg-gray-100 transition-colors duration-200 cursor-pointer"
                        onClick={() =>
                          navigate(`/admin/sales-reports/${report.id}`)
                        }>
                        <td className="px-4 py-2 border-b text-left">
                          {report.grower.company_name}
                        </td>
                        <td className="px-4 py-2 border-b text-left">
                          {report.quarter.toUpperCase()} - {report.year}
                        </td>
                        <td className="px-4 py-2 border-b text-left">
                          {report.quarters_array
                            ? JSON.parse(report.quarters_array)
                                .map(
                                  (q) =>
                        ...
                        <td className="px-4 py-2 border-b text-left">
                          {dateOpen ? format(dateOpen, "dd M, yyyy") : "N/A"}
                        </td>
                        <td className="px-4 py-2 border-b text-left">
                          {report.submission_date || "N/A"}
                        </td>
                        <td className="px-4 py-2 border-b text-left">
